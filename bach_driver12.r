...
stan_model <- paste(out_path, 'bach_script11.stan', sep='')
stan_pars_raw <- c('medb0raw', 'medd1raw', 'slowb0raw', 'slowd1raw',
                   'chem1fastraw', 'chem1medraw', 'chem1slowraw', 
                   'chem2fastraw', 'chem2medraw', 'chem2slowraw')
...
for (i in rows) {
  ...
  if (mode=='zero') {
    ...
    chain_init <- function(){list(
      ...
      chem1fastraw=rtruncnorm(1, 0, 1, 0.2/2.0, priorwide),
      chem1medraw=rtruncnorm(1, 0, 1, 0.1/2.0, priorwide),
      chem1slowraw=rtruncnorm(1, 0, 1, 0.1/2.0, priorwide),
      chem2fastraw=rtruncnorm(1, 0, 1, 2.0/12.0, priorwide),
      chem2medraw=rtruncnorm(1, 0, 1, 3.0/12.0, priorwide),
      chem2slowraw=rtruncnorm(1, 0, 1, 1.0/12.0, priorwide)
    )}
